import argparse
from src.data_process import save_dataset, load_data
import os
from keras.models import load_model
from src.model import RQ_conv_res_bn_model as network
from src.util import show_pic, save_pic
...
parser.add_argument("--test_pic_num", type=int, default=3, help="Which No. of image to be tested.")
...
if __name__ == '__main__':
    ...
    if TEST:
        clean, noise = load_data(DATA_BIN, SIGMA)
        model = network()
        model.load_weights(WEIGHT_PATH)

        if SHOW:
            show_pic(model, clean, noise, PIC_NUM)

        if SAVE:
            if not os.path.exists(SAVE_PATH):
                os.makedirs(SAVE_PATH)
            save_pic(model, clean, noise, PIC_NUM, SAVE_PATH + "/noise_{}_{}.png".format(SIGMA, PIC_NUM))
